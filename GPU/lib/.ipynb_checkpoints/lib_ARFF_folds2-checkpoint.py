...
class Functions:
    ...
    # @staticmethod
    # def ensure_spd(A, eps=1e-6):
    #     """
    #     Eigenvalue-clipping SPD projection.

    #     Input:
    #         A: (..., D, D) raw symmetric or nearly symmetric matrix
    #     Output:
    #         A_spd: (..., D, D) SPD matrix with eigenvalues >= eps
    #     """
    #     B = Functions._symmetrize(A)
    #     vals, vecs = jnp.linalg.eigh(B)
    #     vals_clipped = jnp.maximum(vals, eps)

    #     # Batched Q diag(vals_clipped) Q^T.
    #     return (vecs * vals_clipped[..., None, :]) @ jnp.swapaxes(vecs, -1, -2)

    # @staticmethod
    # def _raw_diffusion_cov(params, x, diff_type):
    #     """
    #     Returns the raw learned covariance representation as a full matrix.
    #     This is NOT guaranteed SPD.
    #     """
    #     x = jnp.atleast_2d(x)
    #     cov_vectors = Functions.beta(params, x)

    #     if diff_type == "diagonal":
    #         D = cov_vectors.shape[1]
    #         return jax.vmap(jnp.diag)(cov_vectors)
    ...
    #         mat = jnp.zeros((D, D), dtype=vec.dtype)
    #         mat = mat.at[LT_idx].set(vec)
    #         return mat + mat.T - jnp.diag(jnp.diag(mat))

    #     return jax.vmap(make_symmetric_matrix)(cov_vectors)

    # @staticmethod
    # def diffusion_cov(params, x, diff_type, spd=True, eps=1e-6):
    #     """
    #     Learned covariance/diffusivity a(x).

    #     If spd=False:
    #         returns raw reconstructed covariance.
    #         Useful for diagnostics and raw RMSE.

    #     If spd=True:
    #         returns SPD-projected covariance.
    #         Use this for NLL, logdet, inverse covariance, and simulation.
    #     """
    #     raw_cov = Functions._raw_diffusion_cov(params, x, diff_type)

    #     if not spd:
    #         return raw_cov

    #     if diff_type == "diagonal":
    #         # Equivalent to eigenvalue clipping for diagonal matrices.
    #         diag_raw = jnp.diagonal(raw_cov, axis1=1, axis2=2)
    #         diag_spd = jnp.maximum(diag_raw, eps)
    #         return jax.vmap(jnp.diag)(diag_spd)

    #     return Functions.ensure_spd(raw_cov, eps=eps)

    # @staticmethod
    # def diffusion(params, x, diff_type, eps=1e-6):
    #     """
    #     Diffusion factor sigma(x) derived from the SPD covariance a(x).

    #     Use this for Euler-Maruyama simulation:
    #         x_{n+1} = x_n + h f(x_n) + sigma(x_n) sqrt(h) xi.
    #     """
    #     cov = Functions.diffusion_cov(params, x, diff_type, spd=True, eps=eps)

    #     if diff_type == "diagonal":
    #         diag_cov = jnp.diagonal(cov, axis1=1, axis2=2)
    #         return jax.vmap(jnp.diag)(jnp.sqrt(diag_cov))
    ...
    #         sqrt_vals = jnp.sqrt(jnp.maximum(vals, eps))
    #         return (vecs * sqrt_vals[None, :]) @ vecs.T

    #     return jax.vmap(matrix_sqrtm)(cov)
...
def nll_loss(drift_param, diff_param, x, r, h, diff_type):
    ...
    delta = r - h * f
    var = Functions.diffusion_cov(diff_param, x, diff_type) * h[:, :, None] 
    ...
    if diff_type == "diagonal":
        ...
        def get_quad_and_logdet(S, d):
            ...
            S_eps = S + 1e-10 * jnp.eye(D)
            sol = jnp.linalg.solve(S_eps, d)
            quad = jnp.dot(d, sol)
...
class ARFFTrain:
    ...
    def ARFF_loop(self, key, hyperparam, x, y, val_split):
        ...
        val_loss = nll_loss(drift_param, diff_param, x_valid, r_valid, h_valid, diff_type)
