...
class PlotResults:
    ...
    def plot_1D(self, true_func, param, x, diff_type, z):
        ...
        if true_func is not None:
            ax.plot(x, true_func(x).reshape(-1,1), color="green")
...
#     diffusion = np.squeeze(true_diffusion(x), axis=0)
...
# def mean_min_loss(true_diffusion, n_pts, validation_split, step_size, xlim, yD):
...
#     MML = 0.5 * integral_result / domain_volume + 0.5 * yD * (1 + np.log(2 * np.pi))

#     SD = np.sqrt(0.5 * yD / (n_pts*(1 - validation_split)))
#     SD_val = np.sqrt(0.5 * yD / (n_pts*validation_split))
    
#     print('Theoretical mean min loss:', MML)
#     print('Loss standard deviation:', SD)
#     print('Validation loss standard deviation:', SD_val)

def integrand_jax(x, step_size, true_diffusion):
    diffusion = jnp.squeeze(true_diffusion(x), axis=0)  # (d, d)
    diffusion_matrix = diffusion @ diffusion.T
    scaled_matrix = step_size * diffusion_matrix
    sign, logdet = jnp.linalg.slogdet(scaled_matrix)
    return logdet

def mean_min_loss(true_diffusion, n_pts, step_size, xlim, yD):
    d = xlim.shape[0]

    # sample uniformly
    key = jax.random.PRNGKey(0)
    samples = jax.random.uniform(key, (n_pts, d))
    samples = xlim[:, 0] + samples * (xlim[:, 1] - xlim[:, 0])

    f = jax.vmap(lambda x: integrand_jax(x, step_size, true_diffusion))
    vals = f(samples)

    integral_result = jnp.mean(vals) * jnp.prod(xlim[:, 1] - xlim[:, 0])

    domain_volume = jnp.prod(xlim[:, 1] - xlim[:, 0])
    MML = 0.5 * integral_result / domain_volume + 0.5 * yD * (1 + jnp.log(2 * jnp.pi))
