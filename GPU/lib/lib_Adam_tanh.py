import jax
import jax.numpy as jnp
import optax
import matplotlib.pyplot as plt
from jax import jit, vmap
from jax import random, grad, value_and_grad
from jax import nn
import jax.scipy.linalg
from functools import partial
jax.config.update("jax_enable_x64", False)
DTYPE = jnp.float32

import time
import math
import numpy as np

EPS = 1e-13
opt = None

class Functions:
    @partial(jit, static_argnums=(2,))
    def S(params, x, i):
        S_ = jnp.tanh(x @ params[f"W{i}"] + params[f"b{i}"])
        return S_
        
    def forward(params, x):
        n_layers = len([k for k in params if k.startswith("W")])
        # apply tanh for all hidden layers except last
        for i in range(1, n_layers):
            x = Functions.S(params, x, i)
        # last layer = linear map
        out = x @ params[f"W{n_layers}"] + params[f"b{n_layers}"]
        return out
        
    def drift(params, x):
        return Functions.forward(params, x)

    def diffusion_cov(params, x, diff_type):
        cov_vectors = Functions.forward(params, x)
        
        if diff_type == "diagonal":
            return vmap(jnp.diag)(jax.nn.softplus(cov_vectors) + EPS)

    def diffusion(params, x, diff_type):
        if diff_type == "diagonal":
            return jnp.sqrt(Functions.diffusion_cov(params, x, diff_type))
    
        diff_vectors = Functions.forward(params, x)

        # arrange vectors into LT matrix
        T = diff_vectors.shape[-1]
        D = int((math.sqrt(1 + 8 * T) - 1) / 2)
        r, c = jnp.tril_indices(D)
        LT = jnp.zeros((x.shape[0], D, D)).at[..., r, c].set(diff_vectors)

        # apply safety on diagonal
        diag_idx = jnp.arange(D)
        LT = LT.at[..., diag_idx, diag_idx].set(jax.nn.softplus(jnp.diagonal(LT, axis1=-2, axis2=-1)) + EPS)
    
        if diff_type == "triangular":
            return LT 

        # make symmetric 
        return jnp.matmul(LT, jnp.swapaxes(LT, -1, -2))


def multivariate_nll_from_scale_tril(scale_tril, delta):
    # solve L y = delta for y
    # jax.scipy.linalg.solve_triangular does not always broadcast over batch, so vmap safely
    def solve_one(Lb, db):
        # lower=True, trans='N'
        return jax.scipy.linalg.solve_triangular(Lb, db, lower=True)

    y = vmap(solve_one)(scale_tril, delta)        
    quad = jnp.sum(y*y, axis=1)                
    diag = jnp.diagonal(scale_tril, axis1=-2, axis2=-1)  
    logdet = 2.0 * jnp.sum(jnp.log(diag), axis=1)       
    return quad, logdet


class AdamTrain:
    def set_opt(hyperparam):
        global opt
        opt = optax.adam(hyperparam["learning_rate"], b1=0.9, b2=0.999, eps=1e-7)

    def nll_loss(drift_param, diff_param, x, r, h, diff_type):
        D = r.shape[1]
        f = Functions.drift(drift_param, x)
        delta = r - h * f   # shape (B, D)

        if diff_type == "diagonal":
            # TF: network outputs std, scale = sqrt(h) * std
            Sigma = jax.nn.softplus(Functions.forward(diff_param, x)) + EPS
            quad = jnp.sum((delta ** 2) / (Sigma * h), axis=1)
            logdet = jnp.sum(jnp.log(Sigma * h), axis=1)
        else:
            scale_tril = Functions.diffusion(diff_param, x, diff_type) * jnp.sqrt(h)[:, :, None]
            if diff_type == "symmetric":
                Sigma = jnp.matmul(scale_tril, jnp.swapaxes(scale_tril, -1, -2))
                scale_tril = jnp.linalg.cholesky(Sigma)
            quad, logdet = multivariate_nll_from_scale_tril(scale_tril, delta)
            
        losses = 0.5 * quad + 0.5 * logdet + 0.5 * D * jnp.log(2.0 * jnp.pi)
        return jnp.mean(losses)
    
    @partial(jit, static_argnames=['diff_type'])
    def train_step(drift_param, diff_param, opt_state_drift, opt_state_diff, x, r, h, diff_type):
        loss, grads = value_and_grad(AdamTrain.nll_loss, argnums=(0, 1))(drift_param, diff_param, x, r, h, diff_type)
        grads_drift, grads_diff = grads
        updates_drift, opt_state_drift = opt.update(grads_drift, opt_state_drift)
        updates_diff, opt_state_diff = opt.update(grads_diff, opt_state_diff)
        drift_param = optax.apply_updates(drift_param, updates_drift)
        diff_param = optax.apply_updates(diff_param, updates_diff)
        
        def global_norm(grads):
            return jnp.sqrt(sum(jnp.sum(jnp.square(p)) for p in jax.tree_util.tree_leaves(grads)))
        grad = global_norm(grads)
        return drift_param, diff_param, opt_state_drift, opt_state_diff, loss, grad
    
    def training_loop(hyperparam, drift_param, diff_param, x, r, h, diff_type, val_split, plot=False):
        #opt = optax.adam(hyperparam["learning_rate"], b1=0.9, b2=0.999, eps=1e-7)
        opt_state_drift = opt.init(drift_param)
        opt_state_diff = opt.init(diff_param)

        batch_size = hyperparam["batch_size"]

        # TF: validation_split = last fraction of data
        n_val = int(x.shape[0] * val_split)
        x_train, r_train, h_train = x[:-n_val], r[:-n_val], h[:-n_val]
        x_val, r_val, h_val = x[-n_val:], r[-n_val:], h[-n_val:]
        N_train = x_train.shape[0]

        losses, val_losses, grads, times = [], [], [], []
        start_time = time.time()
        
        for epoch in range(hyperparam["epochs"]):
            # shuffle training data each epoch (only training, not val)
            perm = jax.random.permutation(jax.random.PRNGKey(epoch), N_train)
            x_shuffled, r_shuffled, h_shuffled = x_train[perm], r_train[perm], h_train[perm]

            for i in range(0, N_train, batch_size):
                x_batch = x_shuffled[i:i+batch_size]
                r_batch = r_shuffled[i:i+batch_size]
                h_batch = h_shuffled[i:i+batch_size]
                drift_param, diff_param, opt_state_drift, opt_state_diff, loss, grad = AdamTrain.train_step(
                    drift_param, diff_param, opt_state_drift, opt_state_diff,
                    x_batch, r_batch, h_batch, diff_type
                )

            val_loss = AdamTrain.nll_loss(drift_param, diff_param, x_val, r_val, h_val, diff_type)
            
            losses.append(float(loss))
            val_losses.append(float(val_loss))
            times.append(time.time() - start_time)
            grads.append(float(grad))
            print(f"\repoch {epoch}: loss = {loss:.4f}, val_loss = {val_loss:.4f}, time = {times[-1]:.4f}s", end='')
        print() 

        if plot:
            plot_loss(times, losses, val_losses)
        
        return drift_param, diff_param, times, losses, val_losses


def init_drift_params(key, K, xD, yD):
    def glorot_uniform(key, shape):
        fan_in, fan_out = shape[0], shape[1]
        limit = jnp.sqrt(6.0 / (fan_in + fan_out))
        return random.uniform(key, shape, minval=-limit, maxval=limit)
        
    """hidden_sizes: list or tuple of hidden layer widths"""
    layer_sizes = [xD] + list(K) + [yD]   # full architecture
    keys = random.split(key, len(layer_sizes) - 1)
    
    params = {}
    for i, (in_dim, out_dim, k) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:], keys), start=1):
        params[f"W{i}"] = glorot_uniform(k, (in_dim, out_dim))
        params[f"b{i}"] = jnp.zeros((out_dim,))
    params["amp"] = jnp.zeros((1, yD))
    return params


def init_diffusion_params(key, K, xD, yD, diff_type):
    def small_uniform(key, shape):
        return random.uniform(key, shape, minval=-1e-2, maxval=1e-2)

    if diff_type != "diagonal":
        yD = int(yD*(yD+1)/2)
        
    """hidden_sizes: list or tuple of hidden layer widths"""
    layer_sizes = [xD] + list(K) + [yD]   # full architecture
    keys = random.split(key, len(layer_sizes) - 1)
    
    params = {}
    for i, (in_dim, out_dim, k) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:], keys), start=1):
        params[f"W{i}"] = small_uniform(k, (in_dim, out_dim))
        params[f"b{i}"] = small_uniform(k, (out_dim,))
    params["amp"] = jnp.zeros((1, yD))
    return params


def plot_loss(times, losses, val_losses):
    plt.plot(times, losses, label='Training Loss')
    plt.plot(times, val_losses, label='Validation Loss')
    plt.xlabel('Time (s)')
    plt.ylabel('Loss')
    plt.title('Training and Validation Loss vs Time')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
