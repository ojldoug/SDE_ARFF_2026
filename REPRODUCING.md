...
All prior scientific qualifications and unrecovered historical search/selection records remain in force. The initial packaging stage performed no fresh training, model predictions or stochastic data generation; subsequent bounded verification is recorded below. A matching input hash and a successful metadata inspection are not evidence of numerical training reproduction.

## Closed bounded verification: four reproduction routes

1. **Archived-output rebuild:** the commands above use Git aggregates and previously validated grid arrays. Hashes, displayed numerical precision and compilation are checked without inference or fitting.
2. **Historical saved-checkpoint evaluation:** supplied selected Ex1/Ex8 seed-0 models passed the original reconstruction checks at `rtol=1e-5, atol=1e-6`. Exact published models require the external bundle. This is distinct from reconstructing them by fresh training, and is not a claim that every checkpoint was reevaluated.
3. **Fresh training with original execution settings:** a clean checkout at `4c5dc6b54af216ebba49477874c955d259d4140f` and separately installed pinned environment executed corrected Ex8 ARFF K128, nominal N80000, seed 0. The first full run failed archived agreement; a later full run recovered the historical scientific outputs but differed from the first. **Strict fresh-process repeatability is not established under the recorded original settings.** See the preserved [first comparison](docs/bounded_reproduction_verification_v1/REPORT.md), [full repeat](docs/current_pipeline_repeat_v1/REPORT.md) and [minimal investigation](docs/minimal_fold0_repeatability_v1/REPORT.md). Incomplete historical runtime records prevent a definitive causal attribution.
4. **Optional autotune-disabled policy:** `--xla_gpu_autotune_level=0`, set before JAX initialization, plus separate initially empty JAX and CUDA cache directories per process. Three minimal fresh processes, each executing twice, were bit-identical. Two complete fresh processes then produced bit-identical scientific arrays, predictions, histories, selected iterations and metrics. Timing/provenance fields and NPZ bytes differ. **Neither full execution matched the historical archive within the unchanged original tolerances.** See [the complete report and command](docs/autotune0_policy_v1/REPORT.md). This optional policy is not the production default and does not prove the historical cause or general determinism.

### Exact tested scope and runtime

Both full-path checks used the packaged `data/ex8_float64_v2.npz`, SHA-256 `6fb009e3d6fa5f241cb1a15f9a9815cbd16ea19c3a9e4ca546ae99bdae091a72`, fixed 80/10/10 split, K128, nominal N80000, seed 0 and **all seven fits**: five nuisance drift regressions, final drift, covariance. Settings: 300 adaptations per regression, lambda=.001, delta=.2, gamma=1, Metropolis on, resampling off, five-fold OOF targets, internal holdout .1, earliest strict internal-validation minimum, no post-selection refit. Final amplitude-fitting count is 72000, each nuisance count 57600; fixed NLL eigenvalue floor .001. Integration/storage is float64; fitting arithmetic remains float32.

Tested software: Python **3.11.15**, JAX/jaxlib/CUDA plugin/PJRT **0.10.2**, NumPy **2.4.6**, NVIDIA CUDA runtime **13.3.29**, cuBLAS **13.6.1.10**, cuSOLVER **12.2.6.9**, cuDNN **9.24.0.43**; driver **595.84**. Hardware: KW61146, NVIDIA RTX A6000 index 0, UUID **GPU-d2cfc73c-186a-6651-e9f1-c6d93a00f0b2**. Exact package/runtime records and mapped library identities are retained in [environment.txt](docs/autotune0_policy_v1/environment.txt) and the report's process records. These are tested versions, not a cross-platform equivalence claim.

The exact tested optional full-fit command and fresh-cache setup follow. Paths describe the isolated verification layout, not an additional dependency on the research checkout; restore the verified bundle into the pinned checkout first. `REPO` supplies only the cooperative lock. `RESOURCES/checkout` supplies all numerical inputs/code, and its sibling `venv` the isolated interpreter. Choose a new absolute `RUN` path. This is documentation, **not authorization for another run**; occupancy and UUID must be checked as in the retained launcher.
...
All unrelated launch settings are shown above; default JAX matmul settings and native explicit HIGHEST Gram/RHS products were retained. No persistent old-policy executable was reused. No solver, precision, random stream, checkpoint policy or tolerance was changed. Detailed program dumps and reconstructed-versus-native matrix distinctions belong to the linked diagnostic report, not the paper.

The optional full runs selected nuisance iterations **1,1,3,4,1**, final drift **4**, covariance **299**; the archive selects **1,1,4,4,1**, **4**, **299**. Test drift/raw covariance/NLL were **0.912831843 / 0.113099642 / -8.879138947**, versus archived **0.912942946 / 0.113298044 / -8.944207191**. These are diagnostic comparisons, not replacement manuscript results.

**Boundary:** all bounded full runs used the packaged dataset. Data generation, other seeds/configurations, other estimators/hardware and reproduction of full result distributions remain unverified. Successful execution, within-policy repeatability, checkpoint evaluation and historical agreement are separate claims. The investigation is closed; no further numerical runs are scheduled by this documentation.

### Artifact release and independent backup

The immutable v1 bundle remains local: 6,978,959,360 bytes (6.50 GiB), 29,009 payload files; see [BUNDLE.json](docs/independent_reproduction_v1/BUNDLE.json). Current release records contain no public URL or verified off-server receipt. The older same-server backup is not independent. Follow [BACKUP.md](docs/independent_reproduction_v1/BACKUP.md) for exact source, checksum and destination verification. Destination selection, license/access review, transfer and verified public download remain author release tasks. No upload or backup is claimed here.
