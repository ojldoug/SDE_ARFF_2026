...
def _validate_standard_config(
    ...
    if data.em_substeps is None:
        raise ValueError("em_substeps must be specified.")

    if data.em_substeps <= 0:
        raise ValueError("em_substeps must be positive.")

    n_fine_steps = data.em_substeps
    h_fine = data.observation_lag / n_fine_steps

    return n_fine_steps, h_fine
...
def generate_standard_em_data(
    ...
    """
    n_fine_steps, h_fine = _validate_standard_config(config)
    data = config.data
