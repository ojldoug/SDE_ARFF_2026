#!/usr/bin/env python3
"""Width-only adapter around the unchanged Experiment 8 Fourier-Adam runners."""
...
RUNNERS = {'joint': 'run_adam_fourier_experiment', 'split': 'run_adam_split_fourier_experiment'}
...
def main(argv=None):
    ...
    p.add_argument('method', choices=RUNNERS)
    p.add_argument('--fourier-frequencies', type=int, choices=[128], default=128)
    ...
    if args.inspect:
        print(json.dumps(dict(method=args.method, runner=RUNNERS[args.method], changes=differences,
                              effective_config=effective), indent=2))
        return
