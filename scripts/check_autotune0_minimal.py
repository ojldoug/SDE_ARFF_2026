#!/usr/bin/env python3
"""Policy gate: one initial model and two executions of its first native step."""
...
reference=dict(np.load(a.checkout/'results/controlled_study_2026/float64_v2/production/baseline/K_128_N_80000/arff/seed_0/artifact.npz'))
data=load_dataset(a.checkout/'data/ex8_float64_v2.npz');ids=reference['train_idx'][f['validation_positions']]
xv,rv,hv=(jnp.asarray(v[ids]) for v in [data.x,data.r,data.h]);yv=rv/hv
repeated={}
for repeat in range(2):
 nextkey,result=step(key,model,x,y);result.amp.block_until_ready()
 pred=fourier_features(result.omega,x)@result.amp;pred.block_until_ready()
 valpred=fourier_features(result.omega,xv)@result.amp;valpred.block_until_ready()
 loss=jnp.mean((valpred-yv)**2);loss.block_until_ready()
 for name,value in dict(key=nextkey,omega=result.omega,amp=result.amp,predictions=pred,validation_predictions=valpred,validation_loss=loss).items():repeated[f'{name}_{repeat}']=np.asarray(value)
