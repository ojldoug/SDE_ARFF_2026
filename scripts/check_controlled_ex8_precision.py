...
print('Generating original-shaped float32 noise on CPU',flush=True)
...
result=dict(N=n,backend='CPU; actual stored GPU data compared explicitly',noise='original PRNG key/full1000×100000×2 float32 shape, same draws cast to float64 for comparison',float32_replay_vs_stored_max_increment_error=float(abs(a-stored).max()),float32_replay_vs_stored_rms_increment_error=float(np.sqrt(np.mean((a-stored)**2))),paired_float32_minus_float64_time_normalized_rmse=float(np.sqrt(np.mean(D**2))),paired_difference_mean=D.mean(0).tolist(),paired_difference_linear_coefficients_rows_intercept_x0_x1=coeff.tolist(),linear_component_rmse=float(np.sqrt(np.mean(fitted**2))),residual_rmse=float(np.sqrt(np.mean((D-fitted)**2))),caveat='CPU replay is not assumed identical to original GPU; backend differences must be inspected before attributing a canonical-data defect.')
with (OUT/'matched_noise_precision_cpu.json').open('x') as f:json.dump(result,f,indent=2)
with (OUT/'matched_noise_precision_cpu.npz').open('xb') as f:np.savez_compressed(f,x=x,stored=stored,cpu32=a,cpu64=b)
