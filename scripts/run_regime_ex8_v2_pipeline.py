"""Persistent amended continuation; never controls the parent h workers."""
import os
...
def wait_parent():
 state(status='waiting_parent_h',phase='parent_h',session='capacity_regime_ex8_h')
 while True:
  verify();s=c.infra.read_json(PARENT/'campaigns/h/state.json')
  if s.get('errors') or s['status']=='failed_workers':raise RuntimeError('Parent campaign worker failure; amendment stopped')
  if s['status']=='complete':return
  if s['status'] not in ['prepared','running','waiting_machine_idle']:raise RuntimeError('Unexpected parent state '+str(s))
  time.sleep(15)
def data():
 ...
 command=[PYTHON,'-B','-u',str(ROOT/'scripts/create_regime_ex8_v2_data.py')]
...
def phase(name):
 ...
 s=c.infra.read_json(control/'state.json')['status'];session='capacity_regime_ex8_v2_'+name
 if s=='prepared':
  assert not (control/'supervisor.log').exists()
  cmd=f'JAX_PLATFORMS=cpu {PYTHON} -B -u {ROOT}/scripts/run_regime_ex8_v2_campaign.py run {name} > {control}/supervisor.log 2>&1'
 ...
 while True:
  ...
  if s.get('errors') or s['status']=='failed_workers':raise RuntimeError('Failed worker in '+name)
  if s['status']=='complete':return
  time.sleep(15)
def analyze(action):
 ...
 command=[PYTHON,'-B','-u',str(ROOT/'scripts/analyze_regime_ex8_v2.py'),action]
...
def main():
 with c.infra.lock_file(CONTROL/'driver.lock'):
  try:
   verify();wait_parent();data();phase('N');phase('h')
   analyze('select');analyze('N');analyze('h');phase('capacity');analyze('capacity')
