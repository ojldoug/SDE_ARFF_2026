...
def main():
    ...
    a=p.parse_args(); w=a.workspace.resolve(); root=w/'checkout'
    if a.mode=='child':
    ...
    try:
        ...
        cmd=[str(w/'venv/bin/python'),'-B',str(Path(__file__).resolve()),'child','--workspace',str(w),'--forbidden-tree',str(a.forbidden_tree),'--lock-dir',str(a.lock_dir),'--gpu',a.gpu]
