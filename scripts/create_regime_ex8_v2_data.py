...
OUT=ROOT/'results/capacity_regime_ex8_v2';PARENT=ROOT/'results/capacity_regime_ex8_v1'
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def write(p,v):
 with p.open('x') as f:json.dump(v,f,indent=2,allow_nan=False)
def main():
 check_assigned_gpu(allow_cpu=False)
 m=json.loads((OUT/'manifest.json').read_text());assert json.loads((PARENT/'campaigns/h/state.json').read_text())['status']=='complete'
 dest=OUT/'data';dest.mkdir(exist_ok=False)
 jax.config.update('jax_enable_x64',True)
 source=load_dataset(PARENT/'dataset_roots/N_640000/h_0.0004/data/ex8.npz')
 ...
 protected={str(PARENT/'dataset_roots/N_640000/h_0.0004/data/ex8.npz'):sha(PARENT/'dataset_roots/N_640000/h_0.0004/data/ex8.npz')}
 ...
 for block in range(8):
  ...
  for t in range(10):
   ...
   if t<4:
    expected=original_lags['records']['0.0004']['noise_block_sha256'][t] if block==0 else prior['extensions'][block-1]['normal_sha256'][t]
   ...
   if t==3:np.testing.assert_array_equal(inc,source.r[start:start+count])
   for h in endpoints:
    if (t+1)*1000==round(h/1e-7):
     assert inc.dtype==np.float64 and np.isfinite(inc).all();endpoints[h].append(inc.copy())
  records.append(dict(block=block,count=count,normal_sha256=hashes,existing_4000_step_prefix_bit_identical=True))
  if block==0:
   ...
   for level in [1,2]:
    bridge=jax.random.normal(jax.random.fold_in(key,2026091400+level),z.shape,dtype=jnp.float64)
    ...
    ref.append(float(np.sqrt(np.mean(((refined-last)/.001)**2))));last=refined
   assert ref[1]<ref[0],ref
   exact=np.asarray(xx)*np.expm1(10000*np.log1p(-1e-7))
   zero=jax.lax.fori_loop(0,10000,lambda i,v:v-jnp.asarray(1e-7,dtype=jnp.float64)*v,xx)
   zero_error=float(np.sqrt(np.mean(((np.asarray(zero-xx)-exact)/.001)**2)))
