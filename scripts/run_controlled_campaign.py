...
import run_controlled_ex8 as adapter
...
def prepare(phase):
 ...
 jobs=[]
 is_stage2=phase.startswith('stage2_');seeds=list(range(5,30)) if is_stage2 else m['stage1_seeds']
 studies={'stage1_capacity_N':['capacity','N'],'stage1_h':['h'],'stage2_capacity':['capacity'],'stage2_N':['N'],'stage2_h':['h'],'calibration':['calibration'],'stage1_adaptation':['adaptation'],'stage2_adaptation':['adaptation']}[phase]
 ...
 # Verify accepted baseline artifact/log bytes against the already accepted summary.
 old=infra.read_json(ROOT/'results/ex8_joint_split_mlp_w27/summary.json')['source_sha256']
 reuse={}
 for j in jobs:
  if j['study']=='capacity' and j['K']==128 and j['N']==80000:
   d=ROOT/'results/production'/BASE_DIRS[j['method']];a=d/f"seed_{j['seed']}_artifacts.npz";l=d/f"seed_{j['seed']}.txt"
   for p in [a,l]:assert adapter.digest(p)==old[str(p.relative_to(ROOT))]
   checked=validate(j,a,l)
 ...
 frozen=['scripts/run_controlled_ex8.py','scripts/run_controlled_campaign.py','scripts/validate_controlled_ex8.py','scripts/run_ex8_split_mlp_w27.py','scripts/run_ex8_k128_campaigns.py']
    ...
    for i,job in enumerate(plan['jobs']):
     ...
     try:
      ...
      if key(job) in plan['reuse']:
       ...
       command=[PYTHON,'-B','-u',str(ROOT/'scripts/run_controlled_ex8.py'),'--job-json',str(directory/'job.json'),'--artifact-path',str(staged)]
