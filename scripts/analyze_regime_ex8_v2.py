...
def select():
 ...
 if not folder.exists():
  ...
  base=replace(d,x=d.x[:100000],r=d.r[:100000],h=d.h[:100000],train_idx=d.train_idx[:80000])
...
def report(phase):
 ...
 if phase=='capacity':
  ...
  lines+=['',json.dumps(interpretation,indent=2),'',verdict,'','Poster candidate only; no poster or manuscript changed. No power-law slope is fitted automatically from five points. See selection.json for frozen N*,h* and unresolved flags.']
