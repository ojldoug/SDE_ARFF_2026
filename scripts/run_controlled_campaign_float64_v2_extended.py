...
def prepare(phase):
 ...
 reuse={}
 for j in jobs:
  if j['study'] in ('capacity','h') and j['K']==128 and j['N']==80000 and j['h']==.0001:
   original={k:v for k,v in dict(j,study='baseline').items() if k!='dataset_root'};d,a,l,ctx=paths(original)
 ...
 if is_stage2:
  predicted=0.
  for j in jobs:
   if key(j) in reuse:continue
   costs=[]
   for seed in m['stage1_seeds']:
    first=dict(j,seed=seed);directory,a,l,ctx=paths(first)
    done=infra.read_json(directory/'complete.json')
    checked=validate(first,a,l,None if done.get('reused') else ctx)
    assert checked['artifact_sha256']==done['artifact_sha256'] and checked['log_sha256']==done['log_sha256']
    costs.append(float(done['algorithm_time'])+120.)
 ...
 frozen=['scripts/run_controlled_campaign_float64_v2_extended.py','scripts/run_controlled_ex8_float64_v2.py','scripts/run_controlled_campaign_float64_v2.py','scripts/run_controlled_ex8.py','scripts/run_controlled_campaign.py','scripts/validate_controlled_ex8.py','scripts/run_ex8_split_mlp_w27.py','scripts/run_ex8_k128_campaigns.py']
