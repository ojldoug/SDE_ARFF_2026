#!/usr/bin/env python3
"""
Validation-only screening of the SPD eigenvalue floor for Experiment 3.

For a fixed ARFF model configuration, the model is fitted exactly once
for each seed. The same fitted model is then evaluated using several
projection floors
...
Current ex3 ARFF configuration used here:

    K = canonical configured value
    M = 50
    resampling = True
...
Selection criterion:
    ...
    mean validation Gaussian NLL across seeds.

Typical use
-----------

    CUDA_VISIBLE_DEVICES=0 \
    python scripts/screen_ex3_spd_epsilon.py \
      --seed 0 \
      --output results/ex3_spd_epsilon_seed0.csv
"""
...
def run_seed(
    ...
    print(
        "Experiment 3 SPD-epsilon validation screen"
    )
    print(
        "=========================================="
    )
