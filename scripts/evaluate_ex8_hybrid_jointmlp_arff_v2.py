...
from src.adam.mlp import MLPParams,AdamMLPModel,predict_mlp
from scripts.run_adam_mlp_experiment import true_function_errors as mlp_errors
...
OUT=BASE/'hybrid_jointmlp_arff/evaluation_v2'
...
def main():
 ...
 write(OUT/'protocol.json',dict(seeds=list(range(30)),dataset_sha256=expected,test_idx_sha256=sha_bytes(d.test_idx),method='Post-hoc same-seed component composition; not independently trained',spd_epsilon=SPD_EPSILON,arithmetic='Accepted float32 evaluation on CPU; archive increments remain float64; native JAX input conversion unchanged',consistency='Exact same-backend component identity; frozen-GPU versus CPU differences reported separately. Primary coefficient metrics inherited exactly from frozen component baseline; direct CPU values also archived.',retraining=False,tuning=False,test_selection=False,script_sha256=sha(__file__)))
 ...
 for seed in range(30):
  ...
  covariance_params=MLPParams(tuple(jnp.asarray(ma[f'covariance_weight_{i}']) for i in range(3)),tuple(jnp.asarray(ma[f'covariance_bias_{i}']) for i in range(3)))
  mlp_model=AdamMLPModel(drift,covariance_params,'symmetric')
  mlp_cpu_drift=float(mlp_errors(mlp_model,x,true_drift=definition.drift,true_diffusion_factor=definition.diffusion_factor)[0])
  arff_cpu_cov=evaluation.true_function_errors(arff,x,true_drift=definition.drift,true_diffusion_factor=definition.diffusion_factor)[1]
  ...
  np.testing.assert_allclose(drift_error,mlp_cpu_drift,rtol=2e-7,atol=2e-8)
  assert cov_error==arff_cpu_cov
  ...
  assert np.isfinite([drift_error,cov_error,likelihood.nll,likelihood.min_raw_eigenvalue]).all()
  check=dict(seed=seed,drift_rmse_minus_same_cpu_component=float(drift_error-mlp_cpu_drift),covariance_rmse_minus_same_cpu_component=float(cov_error-arff_cpu_cov),drift_rmse_minus_baseline=float(drift_error-reference['joint_mlp_drift_rmse'][seed]),covariance_rmse_minus_baseline=float(cov_error-reference['arff_covariance_rmse'][seed]),raw_spd_rate_cpu_minus_baseline=float(likelihood.spd_violation_rate-float(aa['test_raw_spd_violation_rate'])),min_raw_eigenvalue_cpu_minus_baseline=float(likelihood.min_raw_eigenvalue-float(aa['test_min_raw_eigenvalue'])),arff_nll_cpu_minus_baseline=float(arff_cpu.nll-reference['arff_nll'][seed]))
  checks.append(check)
  row=dict(seed=seed,drift_rmse=float(reference['joint_mlp_drift_rmse'][seed]),covariance_rmse=float(reference['arff_covariance_rmse'][seed]),cpu_drift_rmse=drift_error,cpu_covariance_rmse=cov_error,nll=likelihood.nll,raw_spd_violation_rate=float(aa['test_raw_spd_violation_rate']),min_raw_eigenvalue=float(aa['test_min_raw_eigenvalue']),cpu_raw_spd_violation_rate=likelihood.spd_violation_rate,cpu_min_raw_eigenvalue=likelihood.min_raw_eigenvalue,min_projected_eigenvalue=likelihood.min_projected_eigenvalue)
 ...
 statistics={k:desc([v[k] for v in rows]) for k in ['drift_rmse','covariance_rmse','nll','raw_spd_violation_rate','min_raw_eigenvalue','cpu_drift_rmse','cpu_covariance_rmse']}
 differences={}
