...
Code and the review evidence package are available at \url{https://github.com/ojldoug/SDE_ARFF_2026}, branch \texttt{aku/reproducible-pipeline}. The reproduction-package base is commit \texttt{a8809cd}; subsequent review corrections are identified by repository history. Root \texttt{REPRODUCING.md} maps every table and figure to inputs and commands; \texttt{requirements-dev.txt} records the environment. Compact aggregates and validated coefficient grids permit an archive-only CPU rebuild. Large datasets, checkpoints and complete execution archives are not distributed in ordinary Git and have no durable public download identifier yet. The archive-only build verifies tables, figures and PDFs independently of fresh training. Saved-checkpoint evaluation passed the reconstruction checks described above.

A separate bounded verification installed the pinned environment in isolation and executed corrected Experiment 8 ARFF ($K=128$, nominal $N=80000$, seed 0), including all five nuisance fits, final drift and covariance fits. Under the original execution settings, two fresh processes differed. An optional autotuning-disabled policy with fresh compilation caches produced bit-identical scientific outputs in two complete executions, but neither agreed with the historical archive at the unchanged $\mathrm{rtol}=10^{-5}$, $\mathrm{atol}=10^{-6}$ tolerances. This limited software/hardware observation does not establish general determinism or explain the historical discrepancy fully. These checks used packaged data; data generation and other configurations remain unverified. Reported scientific results and production defaults are unchanged. Detailed execution records are provided in the repository reproduction documentation.
